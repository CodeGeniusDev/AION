...
export function RecentTaskRow({ task, index }: { task: RecentTask; index: number }) {
  const Icon = icons[index % icons.length];
  return (
    <article className="grid gap-4 py-4 sm:grid-cols-[minmax(220px,1.4fr)_minmax(120px,.8fr)_90px_88px_38px] sm:items-center">
      ...
      <a href={`/tasks#${task.id}`} className="grid size-9 place-items-center rounded-full border bg-white text-navy hover:bg-[#f7f9fc]" aria-label={`Open ${task.title}`}><ArrowUpRight className="size-4" /></a>
    </article>
  );
}
