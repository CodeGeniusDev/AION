      ...
      <div className="p-4 sm:p-6 lg:p-7">
        ...
        {loading ? (
          <div
            ...
            className="grid gap-5 lg:grid-cols-[minmax(0,1.9fr)_minmax(280px,1fr)]"
          >
            <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {[0, 1, 2].map((item) => (
                <LoadingSkeleton key={item} className="h-[190px]" />
              ))}
            </div>
          ...
          <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.9fr)_minmax(280px,1fr)]">
            <div className="min-w-0 space-y-5">
              <div className="grid min-w-0 gap-5 sm:grid-cols-2 xl:grid-cols-3">
                <SummaryCard
                  title="Total Tasks"
                  description="Tasks coordinated across your workspace"
                  value={data.total_tasks}
                  tone="peach"
                />
                <SummaryCard
                  title="Active Agents"
                  description="Specialists ready to collaborate"
                  value={data.active_agents}
                  tone="blue"
                />
                <SummaryCard
                  title="Saved Memories"
                  description="Useful context retained by AION"
                  value={data.saved_memories}
                  tone="grey"
                />
              </div>
              <section
                id="recent-tasks"
                className="min-w-0 rounded-[24px] border bg-white p-5 sm:p-6"
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-text">
                      View recent AI tasks and results
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-1 rounded-full bg-[#f2f4f7] p-1">
                    {(
                      ["all", "completed", "running", "failed"] as Filter[]
                    ).map((item) => (
                      <button
                        key={item}
                        type="button"
                        ...
                        className={cn(
                          "rounded-full px-3 py-1.5 text-[10px] font-medium capitalize",
                          filter === item
                            ? "bg-white text-aion-text shadow-sm"
                            : "text-muted-text hover:text-aion-text",
                        )}
                      >
                        {item}
                      </button>
              ...
              <section className="rounded-[24px] border bg-white p-5">
                ...
                <div className="mt-3 divide-y">
                  {data.agent_activity.map((agent) => (
                    <AgentActivityRow key={agent.name} agent={agent} />
                  ))}
                </div>
              </section>
            </div>
